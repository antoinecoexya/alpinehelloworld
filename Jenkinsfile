pipeline {
    ...
    stages {
     ...
     post {
       success {
         slackSend (color: '#00FF00', message: "SUCCESSFUL ANTOINE: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
         }
      failure {
            slackSend (color: '#FF0000', message: "FAILED ANTOINE: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
          }   
    }
}
